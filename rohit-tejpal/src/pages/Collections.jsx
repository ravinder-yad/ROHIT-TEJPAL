...
const Collections = () => {
  ...
  return (
    <div className="bg-[var(--color-primary-dark)] min-h-screen pb-16">
      {/* Hero Header */}
      <section className="border-b border-white/10 flex flex-col items-center justify-center text-center px-4 py-16 md:py-24 min-h-[250px]">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 max-w-5xl mx-auto mt-12">
          {collections.map(collection => (
            <Link 
              key={collection._id} 
              to={`/products`}
              className="group relative block overflow-hidden rounded-sm border-[3px] border-white/20 hover:border-[var(--color-gold)]/50 transition-colors duration-500 h-[500px] md:h-[600px]"
            >
              <div className="absolute inset-0 bg-white/5">
                ...
                />
                {/* Overlay Gradient */}
                <div className="absolute inset-0 bg-gradient-to-t from-[var(--color-primary-dark)]/90 via-[var(--color-primary-dark)]/30 to-transparent group-hover:from-black/90 transition-colors duration-500"></div>
              </div>
              ...
              <div className="absolute inset-x-0 bottom-0 flex flex-col items-center justify-end text-center p-8 pb-10">
                ...
                <h3 className="text-white text-2xl md:text-3xl font-serif font-light tracking-widest mb-6 transform group-hover:-translate-y-2 transition-transform duration-500">
                  {collection.name}
                </h3>

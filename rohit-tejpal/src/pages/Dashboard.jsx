    ...
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
      <div className="lg:col-span-2 bg-gradient-to-br from-white to-gray-50 rounded-2xl shadow-sm border border-gray-100 p-8 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h2 className="text-3xl font-extrabold text-[var(--color-primary-dark)] mb-2 tracking-tight">Welcome, {user.name}!</h2>
          <p className="text-gray-500 mb-8 text-sm">Manage your orders, profile, and security settings here.</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            <Link to="/dashboard/cart" className="group block">
              <div className="bg-white p-6 rounded-xl border border-gray-100 flex items-center gap-4 hover:-translate-y-1 hover:shadow-lg hover:border-[var(--color-gold)] transition-all duration-300">
                <div className="w-14 h-14 rounded-full bg-[var(--color-primary-dark)] text-[var(--color-gold)] flex items-center justify-center text-2xl group-hover:bg-[var(--color-gold)] group-hover:text-white transition-colors">
                  <FiShoppingBag />
                </div>
                <div>
                  <span className="block text-3xl font-black text-[var(--color-primary-dark)] mb-0.5 leading-none">{cartItems.length}</span>
                  <span className="text-xs uppercase tracking-widest text-gray-400 font-bold">Items in Bag</span>
                </div>
              </div>
            </Link>
            <Link to="/dashboard/wishlist" className="group block">
              <div className="bg-white p-6 rounded-xl border border-gray-100 flex items-center gap-4 hover:-translate-y-1 hover:shadow-lg hover:border-[var(--color-gold)] transition-all duration-300">
                <div className="w-14 h-14 rounded-full bg-rose-50 text-rose-500 flex items-center justify-center text-2xl group-hover:bg-rose-500 group-hover:text-white transition-colors">
                  <FiHeart />
                </div>
                <div>
                  <span className="block text-3xl font-black text-[var(--color-primary-dark)] mb-0.5 leading-none">{wishlistItems.length}</span>
                  <span className="text-xs uppercase tracking-widest text-gray-400 font-bold">Saved Items</span>
                </div>
              </div>
            </Link>
          </div>
      ...
      <div className="bg-[var(--color-primary-dark)] text-white rounded-2xl shadow-xl p-8 flex flex-col justify-center items-center text-center relative overflow-hidden group hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
        ...
        {recentOrder ? (
          <>
            <div className="bg-white/10 backdrop-blur-sm rounded-lg py-2 px-4 mb-3 border border-white/10 relative z-10">
               <span className="text-xs uppercase tracking-wider text-gray-300 block mb-1">Status</span>
               <strong className="text-sm tracking-wide text-[var(--color-gold)] uppercase">{recentOrder.status}</strong>
            </div>
            <p className="text-2xl text-white font-black mb-8 relative z-10 tracking-tight">₹{recentOrder.totalPrice?.toLocaleString('en-IN')}</p>
          </>
        ) : (
          <p className="text-sm text-gray-300 mb-8 relative z-10 italic">You have no active orders.</p>
        )}
        
        <Link to="/dashboard/orders" className="w-full bg-[var(--color-gold)] hover:bg-white hover:text-[var(--color-primary-dark)] text-[var(--color-primary-dark)] text-xs uppercase tracking-[0.2em] font-bold py-3.5 px-6 rounded-lg transition-colors relative z-10 shadow-lg">
  ...
  return (
    <div className="space-y-6">
      <div className="pb-4 border-b border-gray-200 flex justify-between items-end">
        <div>
          <h2 className="text-2xl font-extrabold text-[var(--color-primary-dark)] tracking-tight">My Orders</h2>
          <p className="text-gray-500 mt-1 text-sm">Track and view your order history.</p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
           ...
           <p className="text-gray-500 font-medium uppercase tracking-widest text-xs">Loading orders...</p>
        </div>
      ) : orders.length > 0 ? (
        <div className="space-y-6">
          {orders.map((order) => (
            <div key={order._id} className="bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-lg hover:border-gray-200 transition-all duration-300 overflow-hidden">
              <div className="bg-gray-50 flex flex-col md:flex-row justify-between md:items-center gap-4 p-5 md:px-6 border-b border-gray-100">
                <div className="grid grid-cols-2 md:flex md:gap-12 gap-y-4">
                  <div>
                    <span className="block text-[10px] text-gray-400 font-black tracking-widest uppercase mb-1">Order ID</span>
                    <p className="text-sm font-bold text-[var(--color-primary-dark)]">#{order._id.substring(order._id.length - 8).toUpperCase()}</p>
                  </div>
                  <div>
                    <span className="block text-[10px] text-gray-400 font-black tracking-widest uppercase mb-1">Date</span>
                  ...
                  <div>
                    <span className="block text-[10px] text-gray-400 font-black tracking-widest uppercase mb-1">Total Amount</span>
                    <p className="text-sm font-black text-[var(--color-primary-dark)]">₹{order.totalPrice?.toLocaleString('en-IN')}</p>
                  </div>
                  <div>
                    <span className="block text-[10px] text-gray-400 font-black tracking-widest uppercase mb-1">Payment</span>
                ...
                <div className="flex flex-col items-start md:items-end gap-2">
                  ...
                  {order.isPaid && (
                    <button 
                      ...
                      className="text-[10px] font-bold uppercase tracking-widest text-[var(--color-gold)] hover:text-[var(--color-primary-dark)] transition-colors mt-1 underline underline-offset-4"
                    >
                      Download Receipt
                    </button>
                  )}
                  ...
                  {order.orderItems.map((item, index) => (
                    <div key={index} className="flex items-center gap-5">
                      ...
                      <div className="flex-1">
                        <h4 className="text-sm font-extrabold text-[var(--color-primary-dark)] mb-1">{item.name}</h4>
        ...
        <div className="text-center py-20 bg-white rounded-2xl border border-gray-100 shadow-sm">
          ...
          <h3 className="text-xl font-extrabold text-[var(--color-primary-dark)] mb-2 tracking-tight">No orders yet</h3>
          <p className="text-gray-500 text-sm max-w-sm mx-auto mb-8">Looks like you haven't placed any orders. Discover our collection.</p>
          <Link to="/products" className="inline-flex items-center justify-center bg-[var(--color-primary-dark)] text-[var(--color-gold)] px-8 py-3.5 rounded-lg text-xs font-black uppercase tracking-[0.15em] hover:bg-black transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5">
            Start Shopping
          </Link>
        </div>
      )}
